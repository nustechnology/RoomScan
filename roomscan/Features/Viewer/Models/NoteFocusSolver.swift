...
nonisolated enum NoteFocusSolver {
    ...
    static func candidateDirections(
        ...
        let interior = interiorCandidate(inward: inward, current: current, elevation: elevation)
        ...
        for degrees in yawOffsets where degrees != 0 {
            let radians = degrees * Float.pi / 180
            candidates.append(rotatedAroundY(interior, radians: radians))
        }
        return candidates
    }

    /// The room-interior viewing direction (`target → eye`), lifted by `elevation`.
    /// Used as the preferred fallback when no candidate has a clear line of sight.
