...
For detailed security guidance, see `docs/security-checklist.md`. Does the change introduce vulnerabilities?
...
Does the change violate the performance requirements defined in `docs/product-requirements.md`?

Check for relevant regressions such as:

- Blocking or expensive work on the main thread.
- Unnecessary loading or retention of large 3D models.
- Full models loaded where thumbnails or lightweight metadata are sufficient.
- Changes that may make 3D interactions such as rotate, pan, or zoom unresponsive.
- Unbounded memory growth or avoidable memory pressure.
- Slow operations that provide no immediate loading or progress feedback.

Only raise performance findings when they are relevant to the changed code and have a plausible user impact. Avoid speculative micro-optimizations.
...
5. Performance: Any relevant regression against product requirements?
```
...
### Performance
- [ ] No relevant regression against product performance requirements
...
- For detailed security review guidance, see `docs/security-checklist.md`
